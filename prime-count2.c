...
int next_work, work_eof = 0;
pthread_mutex_t get_work_lock;

struct work_data get_next_work(void) {
	struct work_data work;
	if (is_sender) {
		pthread_mutex_lock(&get_work_lock);
		if (work_eof) {
		...
		pthread_mutex_unlock(&get_work_lock);
	} else {
...
void* send_work(void* size_ptr) {
	int node_num = *(int*)size_ptr - 1; /* 自分にはこの関数で送らず直接取るので除く */
	...
	while (eof_sent_count < node_num) {
		...
		/* 次のタスクを生成する */
		work = get_next_work();
		if (work.start == 0) eof_sent_count++;
...
void* fetch_work(void* placeholder) {
	(void)placeholder;
	for (;;) {
		struct work_data work;
		/* タスクを取ってくる */
...
void* crunch_work(void* placeholder) {
	...
	for (;;) {
		...
		/* タスクの処理を行う */
		for (i = work.start - 1; i < work.end; i++) {
			check_prime_and_count(i + 1);
		}
	}
	return NULL;
}
...
int main(int argc, char* argv[]) {
	...
	if (is_sender) {
		/* 仕事を配り始める */
		next_work = work_from;
		pthread_mutex_init(&get_work_lock, NULL);
